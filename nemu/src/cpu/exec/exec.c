...
make_group(gp1,
    ...
    /* 0x0f 0x01*/
    make_group(gp7,
               EMPTY, EMPTY, EMPTY, IDEX(lidt2a,lidt),
               EMPTY, EMPTY, EMPTY, EMPTY)
    ...
    opcode_entry opcode_table[512] = {
        ...
        /* 0x90 */ IDEXW(E, setcc, 1), IDEXW(E, setcc, 1), IDEXW(gp7_E, setcc, 1), IDEXW(gp7_E, setcc, 1),
        ...
        /* 0x9c */ IDEXW(gp7_E, setcc, 1), IDEXW(gp7_E, setcc, 1), IDEXW(gp7_E, setcc, 1), IDEXW(E, setcc, 1),
...
{ // exec_real
  ...
  decoding.opcode = opcode;
  set_width(opcode_table[opcode].width); // 设置操作数宽度,mov指令width=0,则在此函数中根据op_size修改为2或4
  idex(eip, &opcode_table[opcode]);
}
